...
const TIMEOUT_MS = Number(process.env.XKX_E2E_TIMEOUT_MS || 45000);
...
ws.on("message", (raw) => {
  ...
  if (gotReady || gotRoom || inGameText) {
    // Wait briefly for look / room.update after ready
    ...
    passTimer = setTimeout(() => {
      ...
      if (!okRoom && !gotReady) {
        done(1, "no_room_signal");
        return;
      }
      done(
        0,
        gotRoom ? "room.update" : inGameText ? "text_in_game" : "ready"
      );
    }, 1500);
  }
});
