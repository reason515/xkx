...
5. **按需跑 e2e**：涉及登录、场景、出口交互或上述新 bug 修复时，在 MUD+网关就绪后执行 `scripts/run-e2e-tests.ps1` 或服务器 `run-e2e-smoke.sh`。
6. **失败则修到绿**：先修实现，再修测试；不要跳过失败用例。
...
# 本地 e2e（需 MUD:8888、网关:3001；可随机注册）
