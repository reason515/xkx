...
int main(object me, string arg)
{
	...
	if (arg == "xingzilin") {
		ob = load_object("/d/newbie_lxsz/xingzilin");
		if (!objectp(ob))
			return notify_fail("（测试）无法加载杏子林。\n");
		me->move(ob);
		tell_object(me, "（测试）你来到杏子林。\n");
		WEBD->mark_web_client(me);
		WEBD->send_room(me, ob);
		return 1;
	}
