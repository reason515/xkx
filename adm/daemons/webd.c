...
void send_room(object me, object env)
{
	...
	foreach (ob in inv) {
		...
		if (ob->is_character()) {
			npcs += ({ sprintf(
				"{\"id\":\"%s\",\"commandId\":\"%s\",\"name\":\"%s\",\"kind\":\"npc\",\"canApprentice\":%d,\"canTrade\":%d,\"canSell\":%d,\"canSteal\":%d,\"isCorpse\":%d,\"canLoot\":%d,\"canLead\":%d,\"canBeg\":%d,\"canPersuade\":%d,\"canWithdraw\":%d,\"canChangeGift\":%d}",
				...
				ob->query("web/can_changegift") ? 1 : 0
			) });
		} else
			items += ({ sprintf(
