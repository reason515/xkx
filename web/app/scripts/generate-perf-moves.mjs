...
/**
 * 无动作目录但武器自带 perform 的武功（太乙剑 do_pfm 拦截 perform 命令）。
 * command 为完整指令；普通动作缺省用 `perform <action>`（无前缀，
 * 服务端按当前武器/准备自动解析门类）。
 */
const EXTRA_SKILLS = {
  "taiyi-jian": {
    name: "太乙剑法",
    actions: [{ action: "bafang", name: "八方风雨", command: "perform sword.bafang" }],
  },
};
...
for (const id of SKILLS) {
  ...
  if (!fs.existsSync(dir) || !fs.statSync(dir).isDirectory()) {
    const extra = EXTRA_SKILLS[id];
    if (extra) {
      out.push({ id, skillName: extra.name, actions: extra.actions });
    } else {
  ...
  for (const name of fs.readdirSync(dir).sort()) {
    ...
    actions.push({
      action,
      name: manual || actionName(full, action) || action,
...
lines.push("  /** 完整指令（缺省无前缀 perform <action>；特殊武功如太乙剑需带门类） */");
...
for (const { id, skillName, actions } of out) {
  ...
  for (const a of actions) {
    lines.push(`    { action: "${a.action}", name: "${a.name}", command: "${a.command}" },`);
  }
  lines.push("  ] },");
}
lines.push("};");
