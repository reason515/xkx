...
test.describe.serial("game smoke", () => {
  ...
  test("菜单以江湖助手说明自动历练", async ({ page }) => {
    ...
    // 按推荐经验从低到高：钓鱼/配药/打怪(均 0) → 悬赏(1) → 石壁(2)
    ...
    await expect(cards.nth(2)).toContainText("打怪练级");
    await expect(cards.nth(3)).toContainText("悬赏任务");
    await expect(cards.nth(4)).toContainText("石壁领悟");
    // 旧刷怪对手列表已移除
