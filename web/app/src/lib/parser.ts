...
function normalizeActionCommand(raw: string): string | null {
  ...
  // `follow none` is the MUD stop-following command, not a scene action chip
  if (verb === "follow" && parts[1]?.toLowerCase() === "none") return null;
...
export function parseSceneryFromDesc(desc: string): Entity[] {
  ...
  const addScenery = (
    ...
    const stripped = normalizeSceneryName(nameRaw)
      ...
      .replace(/^[是在于]/, "")
      .replace(
        /^[一两二三四五六七八九十几數数]*[了着]?[张条个块面扇座间片本封只株棵道行]/,
        ""
      );
...
export function isMyCombatLine(text: string): boolean {
  ...
  if (
    ...
    return true;
  // 对手主动攻向你的行（如「老虎向你扑来」「老虎扑向你」）；
  // 注意避开「对你竖大拇指」等非战斗交互，用攻击动词限定；
  // 兼容「向你的左肩竖劈」这类带部位的被攻击行
  return /(?:扑向你|攻向你|袭向你|逼向你|攻击你|咬向你|抓向你|(?:向你的|向你|朝你的|朝你|往你的|冲你的)(?:.{0,10}?)(?:攻击|劈|拍|砸|扫|踢|戳|捣|轰|撞|抓|咬|扑|砍|刺|打|挥)|向你(?:扑来|袭来|打来))/.test(
    text
  );
}
