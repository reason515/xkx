          ...
          <div className="chips">
            {sceneActions.map((a) => (
              <button
                ...
                type="button"
                className="chip action"
                onClick={() => { if (a.command === 'cun' || a.command === 'qu') setBankingCmd(a.command); else if (a.command.startsWith('__shop__:')) { const cmdId = a.command.slice(9); const npc = state.room.npcs.find(n => (n.commandId || n.id) === cmdId); if (npc) { game.setSelectedEntity(npc); game.openSheet('entity'); } } else sendCommand(a.command); }}
              >
                {a.label}
              </button>
            ))}
