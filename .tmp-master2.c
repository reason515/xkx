...
object connect()
{
	...
	if (err) {
		write("鐜板湪鏈変汉姝ｅ湪淇敼浣跨敤鑰呰繛绾块儴浠界殑绋嬪紡锛岃寰呬細鍐嶆潵銆俓n");
		write(err);
...
{
	efun::shout("绯荤粺鏍稿績鍙戝嚭涓€澹版儴鍙細鍝団€斿摡鈥斿挧鈥擻n");
	efun::shout("绯荤粺鏍稿績鍛婅瘔浣狅細瑕佸綋鏈轰簡锛岃嚜宸变繚閲嶅惂锛乗n");
...
void log_error(string file, string message)
{
	...
	if(wizardp(this_player(1))&&(strsrch(message,"Warning:")==-1)) efun::write("缂栬瘧鏃舵閿欒锛? + message+"\n");
//if(wizardp(this_player(1))) efun::write("缂栬瘧鏃舵閿欒锛? + message+"\n");	

//  涓嶈鍚憀og閲岄潰鍐欎簡,绯荤粺瑕佸穿婧冧簡
...
void destruct_env_of(object ob)
{
	if (!interactive(ob))
		return;
	tell_object(ob, "浣犳墍瀛樺湪鐨勭┖闂磋姣佺伃浜嗐€俓n");
	ob->move(VOID_OB);
}
...
string standard_trace(mapping error, int caught)
{
    ...
    for (i=0, s = sizeof(error["trace"]); i < s; i++) {
		res = sprintf("%s鍛煎彨鏉ヨ嚜锛?s 鐨?%s() 绗?%i 琛岋紝鐗╀欢锛?%O\n",
			res,
...
string error_handler( mapping error, int caught )
{
	...
	if(wizardp(this_player(1)))
		...
		tell_object(this_player(1), "[1;33m浣犲彂鐜颁簨鎯呬笉澶у浜嗭紝鍙槸鍙堣涓嶄笂鏉ャ€?[2;37;0m\n");
